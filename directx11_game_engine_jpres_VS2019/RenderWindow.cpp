...
bool RenderWindow::Initialize(WindowContainer* pWindowContainer, HINSTANCE hInstance, std::string window_title, std::string window_class, int width, int height)
{
	...
	this->handle = CreateWindowEx(0, //Extended Windows style - we are using the default. For other options, see: https://msdn.microsoft.com/en-us/library/windows/desktop/ff700543(v=vs.85).aspx
		...
		pWindowContainer); //Param to create window, could pass in pointer to class in future
...
bool RenderWindow::ProcessMessages()
{
	...
	// PeekMessage is non-blocking, unlike GetMessage
	if (PeekMessage(&msg,		// Where to store message (if one exists) See: https://msdn.microsoft.com/en-us/library/windows/desktop/ms644943(v=vs.85).aspx
		this->handle,			// Handle to window we are checking messages for, pass null for all msgs
...
LRESULT CALLBACK HandleMessageSetup(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_NCCREATE:
	{
		...
		// set user data to be the pointer to our container window
		SetWindowLongPtr(hwnd, GWLP_USERDATA, reinterpret_cast<LONG_PTR>(pWindow));

		// this is how you would get the pointer ^
		// WindowContainer* const pWindow = reinterpret_cast<WindowContainer*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));

		SetWindowLongPtr(hwnd, GWLP_WNDPROC, reinterpret_cast<LONG_PTR>(HandleMsgRedirect));
		OutputDebugStringA("Window has been created\n");
		return pWindow->WindowProc(hwnd, uMsg, wParam, lParam);
	}
	default:
